...
// XP callback - set by the app to award XP on session completion
let onSessionCompleteCallback: ((focusMinutes: number) => void) | null = null

export function setOnSessionCompleteCallback(callback: (focusMinutes: number) => void) {
    ...
    (set, get) => ({
      ...
      completeSession: () => {
        ...
        if (phase === 'focus') {
          ...
          // Notify app to award XP for completing a focus session
          if (onSessionCompleteCallback) {
            try {
              onSessionCompleteCallback(settings.focusDuration)
            } catch (error) {
              console.error('Failed to execute session complete callback:', error)
            }
          }
        }
